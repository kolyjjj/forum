...
let createResponseWhenPostNotFound = (data, id, res, successFunc) => {
  if (lodash.isEmpty(data)) {
    logger.debug('cannot find post with id', id);
    res.status(404).send();
  } else {
    successFunc(data);
  } 
...
router.get('/', (req, res) => {
  postsdb.getAll().then((data)=>{
    ...
    logger.debug('error when getting all posts', err);
...
router.delete('/:id', wrap(async function(req, res, next) {
  try {
    ...
    if (comments !== null && comments.length > 0) {
      logger.debug('deleting comments of posts', comments);
...
router.post('/', (req, res, next) => {
  logger.debug('request body for creating post', req.body);
...
router.put('/:id', (req, res, next)=>{
  logger.debug('request body for updating post', req.body);
